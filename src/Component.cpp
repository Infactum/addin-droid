...
jobject Component::GetActivity() {
    return androidComponentHelper()->GetActivity();
}

jclass Component::FindClass(const std::string &name) {
    auto u16name = toUTF16String(name);
    return androidComponentHelper()->FindClass(reinterpret_cast<const uint16_t *>(u16name.c_str()));
}
...
IAndroidComponentHelper *Component::androidComponentHelper() {
    auto android_iface = static_cast<IAddInDefBaseEx *>(connection)->GetInterface(
            eIAndroidComponentHelper);

    return static_cast<IAndroidComponentHelper *>(android_iface);
}
